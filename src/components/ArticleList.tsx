...
const ArticleList: React.FC<ArticleListProps> = ({ articles, loading,search,selectedSource,selectedCategory,selectedDate }) => {  
...
const filterArticles = () => {
  ...
  if (selectedDate) {
    filtered = filtered.filter((article) => {
      const articleDate = parseDate(article.webPublicationDate) ||
  ...
  if (search) {
    filtered = filtered.filter((article) =>
      ...
      (article.news_desk && article.news_desk.toLowerCase().includes(search.toLowerCase())) ||
      (article.sectionName && article.sectionName.toLowerCase().includes(search.toLowerCase()))
    );
  }
...
useEffect(()=>{
  ...
  return (
    <Box sx={{padding:'20px'}}>
 <Box  sx={{display:'flex', flexDirection:'row', marginBottom:'10px', alignItems:'center', justifyContent:'end'}}>
        <Typography sx={{fontSize:'12px'}}>Showing <b>{filteredArticles.length}</b> from <b>{selectedCategory?1:3} Sources</b></Typography> 
      </Box>
